...
from defense_managers.event_parameters import SDNControllerRequest, SDNControllerResponse, PacketParams, ProcessResult, \
    ManagerActionType, AddFlowAction, ManagerResponse
...
class SDNDefenseApp(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        watch_generated_flows = True  # If flows generated by this class is reported, It is used to test
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None, hard_timeout=0, flags=0, cookie=0,
        ...
        flow_id = cookie
        if cookie == 0:
            flow_id = self._get_next_flow_id(datapath.id)
        ...
        flows[datapath.id][flow_id] = {"packet": mod,  "caller" : caller, "manager" : manager, "group_id":related_group_id}

        return flow_id

    def _get_next_flow_id(self, sw_id):
    ...
    def _packet_in_handler(self, ev):
        ...
        finish = self.new_packet_detected(request_ctx, response_ctx)
        if finish:
            return
        ...
        if actions[0].port != ofproto.OFPP_FLOOD:
            self.adding_default_flow(request_ctx, out_port_list, actions)
            #
    ...
    def packet_in_key(self, *args, **kwargs):
        match = OrderedDict(sorted(args[0].params.default_match.items()))
        dpid = args[0].params.src_dpid
        return "match:"+str(match) + ",dpid:"+str(dpid)

    @cached(cache=TTLCache(maxsize=1024, ttl=1), key=packet_in_key)
    def new_packet_detected(self, request_ctx, response_ctx) -> bool:
        for manager in self.defense_managers:
            manager.on_new_packet_detected(request_ctx, response_ctx)

        return self._process_event_responses(request_ctx, response_ctx)

    @cached(cache=TTLCache(maxsize=1024, ttl=1), key=packet_in_key)
    def adding_default_flow(self, request_ctx, out_port_list, actions) :
        ...
        manager_response.action_list.append(add_flow_action)

        response_ctx.add_response(self, manager_response)

        self._process_event_responses(request_ctx, response_ctx)

    def apply_add_flow_action(self, request_ctx: SDNControllerRequest, action: AddFlowAction, callback_callers, callback_managers):
    ...
    def merge_and_apply_flow_actions(self, request_ctx: SDNControllerRequest, actions : List[AddFlowAction], append_actions: List[AddFlowAction], callback_callers, callback_managers):
        assert len(actions) > 0
        #
        # if actions[0].match is None:
        #     match = request_ctx.params.defrequest_ctx.paramsault_match
        # else:
        #     match = actions[0].match

        if len(actions) + len(append_actions) == 1:
            #actions[0].match = match
            return self.apply_add_flow_action(request_ctx, actions[0], callback_callers, callback_managers)
        ...
        for action in actions:
            ...
            for item in action.actions:
                if isinstance(item, parser.OFPActionOutput) :
                        ports.append(item.port)
            if len(ports) > 0:
                sorted_ports = sorted(ports)
                string_ports = [str(i) for i in sorted_ports]
                ports_string = ":".join(string_ports)
                action_out_ports[action] = ports_string
        ...
        if len(set(action_out_ports.values())) == 1:
            if actions[0].match is None:
                actions[0].match = request_ctx.params.defrequest_ctx.paramsault_match
    ...
    def _process_event_responses(self, request_ctx, response_ctx) -> bool:
        ...
        for _manager, response in response_ctx.responses.items():
            if _manager not in callback_callers:
                callback_callers.append(_manager)

            if _manager not in callback_managers:
                callback_managers.append(_manager)

            for action in response.action_list:
                if action.result_type == ManagerActionType.ADD_FLOW:
                    if action.caller not in callback_callers:
                        callback_callers.append(action.caller)

                    if action.manager not in callback_managers:
                        callback_managers.append(action.manager)
        ...
        # if len(add_flow_actions) == 0:
        #     if len(append_actions) > 0:
        #         self.merge_and_apply_flow_actions(request_ctx, append_actions, [])
        # else:
        for match, actions in add_flow_actions.items():
            self.merge_and_apply_flow_actions(request_ctx, actions, append_actions, callback_callers, callback_managers)
